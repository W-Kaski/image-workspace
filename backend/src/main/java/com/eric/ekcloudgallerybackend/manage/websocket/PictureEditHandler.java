package com.eric.ekcloudgallerybackend.manage.websocket;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.json.JSONUtil;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.eric.ekcloudgallerybackend.manage.websocket.disruptor.PictureEditEventProducer;
import com.eric.ekcloudgallerybackend.manage.websocket.model.PictureEditActionEnum;
import com.eric.ekcloudgallerybackend.manage.websocket.model.PictureEditMessageTypeEnum;
import com.eric.ekcloudgallerybackend.manage.websocket.model.PictureEditRequestMessage;
import com.eric.ekcloudgallerybackend.manage.websocket.model.PictureEditResponseMessage;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        String message = String.format("User %s joined editing", user.getUserName());
    ...
    public void handleEnterEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        ...
        if (!pictureEditingUsers.containsKey(pictureId)) {
            ...
            String message = String.format("User %s start editing", user.getUserName());
            pictureEditResponseMessage.setMessage(message);
            pictureEditResponseMessage.setUser(userService.getUserVO(user));
            // 广播给所有用户
            broadcastToPicture(pictureId, pictureEditResponseMessage);
    ...
    public void handleExitEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        ...
        if (editingUserId != null && editingUserId.equals(user.getId())) {
            ...
            String message = String.format("User %s exit editing", user.getUserName());
            pictureEditResponseMessage.setMessage(message);
            pictureEditResponseMessage.setUser(userService.getUserVO(user));
            broadcastToPicture(pictureId, pictureEditResponseMessage);
        }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
        pictureEditResponseMessage.setType(PictureEditMessageTypeEnum.INFO.getValue());
        String message = String.format("User %s leave editing", user.getUserName());
        pictureEditResponseMessage.setMessage(message);
        pictureEditResponseMessage.setUser(userService.getUserVO(user));
        broadcastToPicture(pictureId, pictureEditResponseMessage);
    }
