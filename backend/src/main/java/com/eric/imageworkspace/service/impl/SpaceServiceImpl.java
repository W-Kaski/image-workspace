...
import com.eric.imageworkspace.model.enums.SpaceLevelEnum;
import com.eric.imageworkspace.model.enums.SpaceRoleEnum;
import com.eric.imageworkspace.model.enums.SpaceTypeEnum;
...
import com.eric.imageworkspace.mapper.SpaceMapper;
import com.eric.imageworkspace.service.SpaceUserService;
...
public class SpaceServiceImpl extends ServiceImpl<SpaceMapper, Space>
    ...
    public long addSpace(SpaceAddRequest spaceAddRequest, User loginUser) {
        ...
        if (SpaceLevelEnum.COMMON.getValue() != space.getSpaceLevel() && !userService.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "No Privilege to add space");
        }
    ...
    public void validSpace(Space space, boolean add) {
        ...
        if (add) {
            if (StrUtil.isBlank(spaceName)) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "space name is empty");
            }
            if (spaceLevel == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "space level is empty");
            }
            if (spaceType == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "space type is empty");
            }
        }
        // 修改数据时，空间名称进行校验
        if (StrUtil.isNotBlank(spaceName) && spaceName.length() > 30) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "space name is longer than 30 characters");
        }
        // 修改数据时，空间级别进行校验
