<template>
  <div class="space-category-analyze">
    <a-card title="Space Category Analyze">
...
const fetchData = async () => {
  ...
  if (res.data.code === 0 && res.data.data) {
    ...
    message.error('Fail to fetch space category analyze data，' + res.data.message)
  }
  loading.value = false
}
...
const options = computed(() => {
  ...
  return {
    ...
    legend: { data: ['Picture Count', 'Total Size (MB)'], top: 'bottom' },
    ...
    series: [
      ...
      { name: 'Total Size (MB)', type: 'bar', data: sizeData, yAxisIndex: 1 },
    ],
  }
})
</script>
